...
session_start();
 ...
 if(isset($_POST['delete'])){
    ...
    $delete_cart_item = $connDB->prepare("DELETE FROM `cart` WHERE id = ?");
    $delete_cart_item->execute([$cart_id]);
 }
 
 if(isset($_GET['delete_all'])){
    $delete_cart_item = $connDB->prepare("DELETE FROM `cart` WHERE user_id = ?");
 ...
 if(isset($_POST['update_qty'])){
    ...
    $update_qty = $connDB->prepare("UPDATE `cart` SET quantity = ? WHERE id = ?");
...
?>
        ...
        <div class="cart-wish-total">
            ...
            <a href="../userFunctions/checkout.php" class="btn <?= ($grand_total > 1)?'':'disabled'; ?>">proceed to
                checkout</a>
        </div>
